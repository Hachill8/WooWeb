...
var maxlimit, hidetout=true, hidetin=true, hidet=true;/*最長 室外是否先隱藏 棚內是否先隱藏 氣象站是否先隱藏 */
	...
	setTimeout(function() {
		...
		lineChart4 = new Chart(ctxH, {
			...
			options: {
				/*legend: {
					labels: {
					   filter: function(datasets) {
						  if (datasets.text === '-999') return null; //only show when the label is cash
					   }
					}
				 },*/
				maintainAspectRatio: false,
...
$(document).ready(function() {
	...
	{
		arr1 = url.split('?');
		arr2 = arr1[1].split('&');/*arr2[1]=感測器選取 arr2[2]=開始日期 arr2[3]=結束日期 arr[4]=裝置比較 */
		var data = { 起始日期: arr2[2], 結束日期: arr2[3]};
		var sen = [];
		sen = arr2[1].split(',');
		//var data = { 起始日期: '20201115', 結束日期: '20201116'};
		...
		/*把mainpage打開history表單時選的紀錄傳到這一頁 */
		/*日期 */
		$("#Date2").datepicker({
			dateFormat: 'yy-mm-dd'
		...
		/*感測器sensorselect和裝置environmentselect */
		...
		$.each(arr2[5].split(","), function(i,e){
			$("#vegetselect option[value='" + e + "']").prop("selected", true);
		});
		console.log(arr2[5]);
		/*$.each(arr2[6].split(","), function(i,e){
			$("#environmentselect option[value='" + e + "']").prop("selected", true);
		});*/
		/*$("#sensorselect").multiselect({
			nonSelectedText:'請選擇感測器'
		  });*/
...
function submithistory(){/*重新查詢button按下後用新的href先重新載入頁面，然後用新的href後面接的條件重新呼叫ajax */
	...
	window.location.href = window.location.host+"?sensor=&"+sensorselectsubmit+"&"+date+"&"+date2+"&"+environmentselectsubmit;
	/*lineChart.clear();
	setTimeout(function() {
		var ctxT = document.getElementById('lineChart')
		ctxT.height = 300
		ctxT.width = 500
		var data = {
				labels: Wlabels,
	...
	setTimeout(function() {
		var ctxH = document.getElementById('lineChart4')
		ctxH.height = 300
		ctxH.width = 500
		var dataH = {
			labels: Wlabels,
